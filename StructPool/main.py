...
class Classifier(nn.Module):
    def __init__(self, prior_knowledge_size=0):
        ...
        if out_dim == 0:
            if cmd_args.gm == 'DGCNN':
                out_dim = self.s2v.dense_dim + prior_knowledge_size
            else:
        ...
        # self.w = nn.Linear(175, 10, bias=False)
    ...
    def forward(self, batch_graph, batch_prior_knowledge, batch_prior_knowledge_slice=0):
        node_feat, labels = self.PrepareFeatureLabel(batch_graph)
        embed = self.s2v(batch_graph, node_feat, None)

        if batch_prior_knowledge is not None:
            batch_prior_knowledge = np.array(batch_prior_knowledge)
            if batch_prior_knowledge_slice != 0:
                global_info = torch.FloatTensor(batch_prior_knowledge[:, :1]).cuda()
                subgraph_info = torch.FloatTensor(batch_prior_knowledge[:, 1:]).cuda()
                embed = torch.cat((embed, subgraph_info), dim=1)
            else:
                subgraph_info = torch.FloatTensor(batch_prior_knowledge).cuda()
                embed = torch.cat((embed, subgraph_info), dim=1)
...
def loop_dataset(g_list, classifier, sample_idxes, optimizer=None,
                 bsize=cmd_args.batch_size, prior_knowledge=None, prior_knowledge_slice=None):
    total_loss = []
    ...
    for pos in pbar:
        ...
        batch_prior_knowledge = None
        if prior_knowledge is not None:
            batch_prior_knowledge = [prior_knowledge[idx] for idx in selected_idx]
        targets = [g_list[idx].label for idx in selected_idx]
        all_targets += targets
        logits, loss, acc = classifier(batch_graph, batch_prior_knowledge, prior_knowledge_slice)
...
if __name__ == '__main__':
    ...
    train_graphs, test_graphs, prior_knowledge_train, prior_knowledge_test, prior_knowledge_slice = load_data()
    ...
    if prior_knowledge_train is not None:
        classifier = Classifier(len(prior_knowledge_train[0]) - prior_knowledge_slice)
    else:
    ...
    for epoch in range(cmd_args.num_epochs):
        ...
        avg_loss = loop_dataset(
            train_graphs, classifier, train_idxes, optimizer=optimizer, prior_knowledge=prior_knowledge_train, prior_knowledge_slice=prior_knowledge_slice)
        ...
        test_loss = loop_dataset(
            test_graphs, classifier, list(range(len(test_graphs))), prior_knowledge=prior_knowledge_test, prior_knowledge_slice=prior_knowledge_slice)
        if not cmd_args.printAUC:
            test_loss[2] = 0.0
